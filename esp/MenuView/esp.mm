...
static float espDistance = 400.0f; // дальность прорисовки ESP
...
- (void)setupMenuUI {
    ...
    // ESP Distance Slider
    UILabel *espDistLabel = [[UILabel alloc] initWithFrame:CGRectMake(15, 50, 300, 20)];
    espDistLabel.text = @"ESP Distance (m): 400";
    espDistLabel.textColor = [UIColor whiteColor];
    espDistLabel.font = [UIFont systemFontOfSize:13];
    espDistLabel.tag = 999;
    [settingTabContainer addSubview:espDistLabel];

    UISlider *espDistSlider = [[UISlider alloc] initWithFrame:CGRectMake(15, 75, 400, 20)];
    espDistSlider.minimumValue = 50.0;
    espDistSlider.maximumValue = 1000.0;
...
- (void)espDistChanged:(UISlider *)sender {
    ...
    // Обновляем лейбл
    UILabel *lbl = (UILabel *)[sender.superview viewWithTag:999];
    if (lbl) lbl.text = [NSString stringWithFormat:@"ESP Distance (m): %.0f", espDistance];
}
...
- (void)renderESPToLayers:(NSMutableArray<CALayer *> *)layers {
    ...
    for (int i = 0; i < coutValue; i++) {
        ...
        // рендерим всех кто прошёл espDistance фильтр
