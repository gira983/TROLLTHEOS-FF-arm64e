...
{
    [_timer invalidate];
    // Запускаем таймер на фоне чтобы не нагружать main runloop
    // updateSpeedLabel через dispatch_async(main) обновляет label
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
        self->_timer = [NSTimer timerWithTimeInterval:UPDATE_INTERVAL
                                              target:self
